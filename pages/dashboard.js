              ...
              <div key={mkt.key} style={{ background: hasAccess(mkt) ? 'var(--bg2)' : 'var(--bg3)', border: '0.5px solid var(--border)', borderRadius: 10, padding: '14px 18px', flex: 1, minWidth: 180, opacity: hasAccess(mkt) ? 1 : 0.6 }}>
                ...
                {hasAccess(mkt) ? (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {mkt.tags.map(tag => <span key={tag} style={{ fontSize: 11, padding: '2px 8px', background: 'var(--green3)', color: 'var(--green2)', borderRadius: 4 }}>{tag}</span>)}
                  </div>
                ) : (
                  <div>
                    <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 8 }}>{t.availableIn} {mkt.plans[0]}</div>
                    <button onClick={() => router.push('/signup')} style={{ fontSize: 12, padding: '5px 12px', background: 'transparent', border: '0.5px solid var(--border2)', color: 'var(--text2)', borderRadius: 6, cursor: 'pointer' }}>{t.upgrade}</button>
                  </div>
                )}
              </div>
            ))}
